  ...
  // ────────────────────────────────────────────── 2
  {
    ...
    beats: [
      ...
      },
      {
        h: 'Q・K・V — 質問と見出しと中身',
        simple: {
          body: `各トークンは3つの役を持ちます。<strong>Q</strong>（いま何を探しているか）、<strong>K</strong>（自分は何者か、という見出し）、<strong>V</strong>（自分が渡せる中身）。QとKを照らし合わせて「どれくらい注目するか」を決め、その重みでVを混ぜ合わせる。図書館で目次を見て必要な本だけ開くのに似ています。`,
          speech: '各トークンは3つの役を持ちます。キューは、いま何を探しているか。ケーは、自分は何者かという見出し。ブイは、自分が渡せる中身。キューとケーを照らし合わせて、どれくらい注目するかを決め、その重みでブイを混ぜ合わせます。',
        },
        deep: {
          body: `<code>Attention(Q,K,V) = softmax(QKᵀ / √d_k) V</code>、ただし <code>Q=XW_Q, K=XW_K, V=XW_V</code>。<code>√d_k</code> によるスケーリングは内積の分散増大でsoftmaxが飽和するのを防ぎます。ヘッドを複数並べる<strong>多頭注意</strong>で、係り受けや照応といった異なる関係を分業させる。計算量は系列長 <code>n</code> に対し <code>O(n²d)</code> で、<strong>これが長文コンテキストの主要コスト要因</strong>です。`,
    ...
    beats: [
      {
        h: '砂嵐から始める（拡散モデル）',
        simple: {
          body: `画像生成AIは、まっさらな<strong>ノイズ（砂嵐）</strong>から始めます。「このノイズをほんの少しだけきれいにする」処理を何十回も繰り返して、だんだん絵を浮かび上がらせる。石を削って像を彫り出す作業に似ています。文章生成の「1トークンずつ足す」とはまったく別の原理です。`,
          speech: '画像生成エーアイは、まっさらなノイズから始めます。このノイズをほんの少しだけきれいにする処理を何十回も繰り返して、だんだん絵を浮かび上がらせます。石を削って像を彫り出す作業に似ています。',
        },
        deep: {
